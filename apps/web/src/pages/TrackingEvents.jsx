...
export default function TrackingEvents() {
  ...
  const q = sp.get("q") || ""; // search by shipment_id (simple MVP)
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const res = await apiFetch(`${TRACKING_API}/events?limit=${limit}&offset=${offset}`, { auth: true });
      const list = res?.items || [];
      setItems(list);
      setTotal(res?.total ?? null);
    } catch (e) {
      setError(e?.message || "Failed to load tracking events");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (!qq) return items;
    return items.filter((ev) => String(ev.shipment_id || "").toLowerCase().includes(qq));
  }, [items, q]);
  ...
  const goPrev = () => setSp((prev) => new URLSearchParams({ ...Object.fromEntries(prev), offset: String(Math.max(0, offset - limit)) }));
  const goNext = () => setSp((prev) => new URLSearchParams({ ...Object.fromEntries(prev), offset: String(offset + limit) }));
  const setLimit = (v) => setSp((prev) => new URLSearchParams({ ...Object.fromEntries(prev), limit: String(v), offset: "0" }));
      ...
      <div className="mb-4 flex flex-col lg:flex-row lg:items-center gap-3">
        <form onSubmit={submitSearch} className="flex-1 relative">
          ...
          <input
            ...
            placeholder="Filter by shipment_id (client-side)"
            value={qInput}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="text-xs text-slate-500">
            Offset: {offset} · Limit: {limit} · Showing: {filtered.length}
          </div>
        </div>
...
{JSON.stringify(ev.payload, null, 2)}
      ...
      <div className="mt-3 text-xs text-slate-500">
        MVP: search/filter is client-side. If needed, we can add server-side filters (shipment_id, event_type, date range).
      </div>
    </DashboardLayout>
  );
}
