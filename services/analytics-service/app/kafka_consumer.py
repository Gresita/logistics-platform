...
def _handle_status_changed(payload: dict):
    ...
    try:
        ...
        if s is None:
            ...
            # --- route stats update when delivered ---
            if (status or "").upper() == "DELIVERED" and s.created_ts is not None and origin and destination:
                duration_hours = (ts - s.created_ts).total_seconds() / 3600.0
                if duration_hours >= 0:
                    rs = (
                        db.query(RouteStats)
                          .filter(RouteStats.origin == origin, RouteStats.destination == destination)
                          .first()
                    )
                    now = _now_utc_naive()
                    if rs is None:
                        rs = RouteStats(origin=origin, destination=destination, samples=1, mean_hours=float(duration_hours), last_updated=now)
                        db.add(rs)
                    else:
                        new_samples = rs.samples + 1
                        rs.mean_hours = (rs.mean_hours * rs.samples + float(duration_hours)) / new_samples
                        rs.samples = new_samples
                        rs.last_updated = now
